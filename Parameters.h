  ...
  public:
    void set_from_cmdline(int argc, char* argv[]) {
      ...
      desc.add_options()
        ...
        ("tolerance", po::value<float>(), "Set comparison tolerance.")
        ("kernel-names", po::value<std::vector<std::string>>()->multitoken()->
                             zero_tokens()->composing(), "List of kernel names")
        ;
    ...
    std::string get_reference_file() {
      return reference_file;
    }
    std::string get_kernels_file() {
      return kernels_file;
    }
    ...
    void print() {
      // TODO: should take a stream as argument
      std::cerr << std::endl;
      std::cerr << "********************************************" << std::endl;
      std::cerr << "|   PARAMETERS                             |" << std::endl;
      std::cerr << "--------------------------------------------" << std::endl;
      std::cerr << "Reference file is: " << reference_file << std::endl;
      std::cerr << "Kernels file is: " << kernels_file << std::endl;
      std::cerr << "Using a tolerance of: " << tolerance << std::endl;
      std::cerr << "Kernels to be computed are: " << std::endl;
